...
class MPCController:
    def __init__(self, T=0.2, N=10, v_max=0.5, omega_max=np.pi / 3.0, a_max=0.3):
        self.T = T
        self.N = N
        self.v_max = v_max
        ...
        self.f = lambda x_, u_: ca.vertcat(
            *[u_[0] * ca.cos(x_[2]) - u_[1] * ca.sin(x_[2]), u_[0] * ca.sin(x_[2]) + u_[1] * ca.cos(x_[2]), u_[2]]
        )
        self.f_np = lambda x_, u_: np.array(
            [u_[0] * np.cos(x_[2]) - u_[1] * np.sin(x_[2]), u_[0] * np.sin(x_[2]) + u_[1] * np.cos(x_[2]), u_[2]]
        )
        ...
        # Boundaries
        # self.opti.subject_to(self.opti.bounded(-10.0, self.x, 10.0))
        # self.opti.subject_to(self.opti.bounded(-10.0, self.y, 10.0))
