...
class ParamBox(Box):
                ...
                for row_index, (param, value) in enumerate(self._tbkdata.param_data.items()):
                    with dpg.table_row(parent=table_tag, tag=param):
                        ...
                        for item in value:
                            if item == "value":
                                tag = param + "_" + item
                                self._diycomponents.param_input(outer_instance=self._diycomponents, data=self._uidata,
                                                                tbkdata=self._tbkdata).new_input(
                                    tag=tag,
                                    value=value[item],
                                    type=_type,
                                    info=_info,
    ...
    def update(self):
        for k, v in self._tbkdata.param_data.items():
            # print('k: ',k)
            # print('v: ',v)
